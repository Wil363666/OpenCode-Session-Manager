...
def save_config(storage_path: Optional[Path] = None, theme: Optional[str] = None):
    ...
    try:
        ...
        save_data = {
            ...
            if isinstance(existing["storage_path"], str)
            else (str(existing["storage_path"]) if existing["storage_path"] else None),
...
def _backup_session_data(session_id: str, project_id: str = None) -> dict:
    ...
    backup["diff_file"] = _backup_file(
        STORAGE_PATH / "session_diff" / f"{session_id}.json"
    )
...
def _cleanup_desktop_cache_by_worktree(worktree: str):
    ...
    try:
        ...
        worktree_b64 = (
            base64.b64encode(worktree.encode("utf-8")).decode("utf-8").rstrip("=")
        )
                ...
                try:
                    ...
                    if "sessionTabs" in layout_data:
                        ...
                        layout_data["sessionTabs"] = {
                            k: v
                            for k, v in original_tabs.items()
...
def _migrate_desktop_cache_worktree(old_worktree: str, new_worktree: str):
    ...
    try:
        ...
        old_b64 = (
            base64.b64encode(old_worktree.encode("utf-8")).decode("utf-8").rstrip("=")
        )
        new_b64 = (
            base64.b64encode(new_worktree.encode("utf-8")).decode("utf-8").rstrip("=")
        )
...
def _project_needs_fix(data: dict) -> bool:
    ...
    if len(project_id) != 40 or not all(
    ...
    for project_file in project_dir.glob("*.json"):
        try:
            ...
            if (
                ...
                and search.lower() not in worktree.lower()
            ):
                continue

            project_session_dir = session_dir / project_id
            session_count = (
                len(list(project_session_dir.glob("*.json")))
...
async def fix_all_projects():
    ...
    for project_file in list(project_dir.glob("*.json")):
        try:
            ...
            errors.append(
                {"file": project_file.stem, "error": "Failed to read project file"}
            )
            continue
        ...
        try:
            ...
            is_valid_git_id = len(old_project_id) == 40 and all(
                c in "0123456789abcdef" for c in old_project_id.lower()
            )
            ...
            if not is_valid_git_id and worktree and Path(worktree).exists():
                ...
                if success and new_project_id and new_project_id != old_project_id:
                    ...
                    if old_session_dir.exists():
                        if new_session_dir.exists():
                            ...
                            for session_file in old_session_dir.glob("*.json"):
                                shutil.move(
                                    str(session_file),
                                    str(new_session_dir / session_file.name),
                                )
                    ...
                    errors.append(
                        {
                            "file": old_project_id,
                            "error": error or "Failed to ensure git repo",
                        }
                    )
                    continue
    ...
    for session_file in session_dir.glob("*.json"):
        try:
            ...
            if (
                ...
                and search.lower() not in session_id.lower()
            ):
                continue
...
async def search_all_sessions(search: str = ""):
    ...
    for project_folder in session_base_dir.iterdir():
        ...
        project_info = project_map.get(
            project_id, {"id": project_id, "name": project_id, "worktree": ""}
        )
        ...
        for session_file in project_folder.glob("*.json"):
            try:
                ...
                if (
                    ...
                    and search_lower not in session_id.lower()
                    and search_lower not in project_info["name"].lower()
                ):
                    continue
...
async def check_repair_project(project_id: str, request: Request):
    ...
    try:
        data = await request.json()
        git_user_name = (
            data.get("git_user_name", "").strip() if data.get("git_user_name") else None
        )
        git_user_email = (
            data.get("git_user_email", "").strip()
            if data.get("git_user_email")
            else None
        )
    except Exception:
    ...
    is_valid_git_id = len(project_id) == 40 and all(
        c in "0123456789abcdef" for c in project_id.lower()
    )
    ...
    if not is_valid_git_id:
        report["issues_found"].append(
            f"Project ID '{project_id}' is not a valid git commit hash"
        )

        if worktree and Path(worktree).exists():
            success, git_project_id, error = _ensure_git_repo(
            ...
            if error == "NEEDS_GIT_CONFIG":
                ...
                return {
                    ...
                    "message": "Git user configuration required",
                    "worktree": worktree,
                }
            ...
            if success and git_project_id:
                ...
                if new_project_file.exists() and new_project_id != old_project_id:
                    report["errors"].append(
                        f"Cannot migrate: project with ID {new_project_id} already exists"
                    )
                    return report
                ...
                if old_project_id != new_project_id:
                    ...
                    report["fixes_applied"].append(
                        f"Migrated project ID: {old_project_id} -> {new_project_id}"
                    )
                ...
                if old_session_dir.exists() and old_project_id != new_project_id:
                    ...
                    for session_file in old_session_dir.glob("*.json"):
                        ...
                        report["fixes_applied"].append(
                            f"Moved session file: {session_file.name}"
                        )
                    ...
                    if not any(old_session_dir.iterdir()):
                        old_session_dir.rmdir()
                        report["fixes_applied"].append(
                            f"Removed empty session directory: {old_project_id}"
                        )
            else:
    ...
    if session_dir.exists():
        for session_file in session_dir.glob("*.json"):
            try:
                ...
                report["errors"].append(
                    f"Failed to process session {session_file.stem}: {e}"
                )
    ...
    if worktree and DESKTOP_CACHE_PATH and DESKTOP_CACHE_PATH.exists():
        try:
            ...
            if "server.v3" in cache_data:
                ...
                server_data = {
                    "list": [],
                    "projects": {"local": [{"worktree": worktree, "expanded": True}]},
                }
                cache_data["server.v3"] = json.dumps(server_data)
                cache_modified = True
                report["fixes_applied"].append(
                    "Created server.v3 in Desktop cache with project"
                )
        ...
        if message_part_dir.exists():
            ...
            for part_data in part_files:
                ...
                if part_type == "text":
                    ...
                    tool_name = part_data.get("toolInvocation", {}).get(
                        "toolName", "unknown"
                    )
...
def _ensure_git_repo(
    ...
    try:
        ...
        if not git_dir.exists():
            result = subprocess.run(
                ["git", "init"], cwd=worktree, capture_output=True, text=True
            )
            if result.returncode != 0:
        ...
        if not project_id:
            ...
            if not any(Path(worktree).iterdir()) or all(
                p.name == ".git" for p in Path(worktree).iterdir()
            ):
                gitkeep.touch()
            ...
            result = subprocess.run(
                [
                    ...
                    "commit",
                    "-m",
                    "Initial commit (created by OpenCode Session Manager)",
                    "--allow-empty",
                ],
                cwd=worktree,
                capture_output=True,
...
async def create_project(request: Request):
    ...
    git_user_email = (
        data.get("git_user_email", "").strip() if data.get("git_user_email") else None
    )
    ...
    if not worktree:
        raise HTTPException(
            status_code=400,
            detail="Worktree path is required for OpenCode compatibility",
        )
    ...
    success, project_id, error = _ensure_git_repo(
        worktree, git_user_name, git_user_email
    )
    if not success or not project_id:
        ...
        raise HTTPException(
            status_code=400, detail=error or "Failed to initialize git repository"
        )
...
async def update_project(project_id: str, request: Request):
    ...
    if "worktree" in data and data["worktree"] != old_worktree:
        ...
        if new_worktree and Path(new_worktree).exists():
            ...
            success, git_project_id, error = _ensure_git_repo(
                new_worktree, git_user_name, git_user_email
            )
            if error == "NEEDS_GIT_CONFIG":
                # Return special response indicating git config is needed
                return {
        ...
        if migrated and new_project_id:
            ...
            if new_project_file.exists():
                ...
                try:
                    ...
                    target_project_name = existing_project_data.get(
                        "name", new_project_id
                    )
...
async def delete_projects(request: Request):
    ...
    if deleted:
        description = (
            f"Delete {len(deleted)} project(s): {', '.join(project_names[:3])}"
        )
...
async def delete_sessions(request: Request):
    ...
    if deleted:
        description = (
            f"Delete {len(deleted)} session(s): {', '.join(session_names[:3])}"
        )
...
async def get_undo_status():
    ...
    return {
        "available": True,
        "description": last_entry.get("description", "Unknown operation"),
        "count": len(undo_history),
    }
...
async def perform_undo():
    ...
    if entry["type"] == "delete_sessions":
        ...
        try:
            ...
            original_project_id = entry.get(
                "old_project_id_in_file", entry["source_project_id"]
            )
        ...
        try:
            ...
            if session_dir.exists() and any(session_dir.glob("*.json")):
                errors.append(
                    {
                        "id": entry["project_id"],
                        "error": "Cannot undo: project now has sessions",
                    }
                )
            else:
                if project_file.exists():
        ...
        for diff_file in session_diff_dir.glob("*.json"):
            ...
            if session_id not in valid_sessions:
                ...
                orphans["session_diffs"].append(
                    {"path": str(diff_file), "session_id": session_id, "size": size}
                )
                orphans["total_size"] += size
        ...
        for todo_file in todo_dir.glob("*.json"):
            ...
            if session_id not in valid_sessions:
                ...
                orphans["todos"].append(
                    {"path": str(todo_file), "session_id": session_id, "size": size}
                )
                orphans["total_size"] += size
        ...
        for session_msg_dir in message_dir.iterdir():
            if session_msg_dir.is_dir():
                ...
                if session_id not in valid_sessions:
                    ...
                    dir_size = sum(
                        f.stat().st_size
                        for f in session_msg_dir.rglob("*")
                        if f.is_file()
                    )
        ...
        for msg_part_dir in part_dir.iterdir():
            if msg_part_dir.is_dir():
                ...
                if message_id not in valid_messages:
                    # Entire directory is orphaned
                    dir_size = sum(
                        f.stat().st_size for f in msg_part_dir.rglob("*") if f.is_file()
                    )
                        ...
                        try:
                            ...
                            if internal_session_id:
                                if internal_session_id not in valid_sessions:
                                    issues.append(
                                        f"sessionID '{internal_session_id}' doesn't exist"
                                    )
                                elif (
                                    expected_session_id
                                    and internal_session_id != expected_session_id
                                ):
                                    issues.append(
                                        f"sessionID mismatch: expected '{expected_session_id}', got '{internal_session_id}'"
                                    )

                            # Check if messageID in part matches the directory
                            if (
                                internal_message_id
                                and internal_message_id != message_id
                            ):
                                issues.append(
                                    f"messageID mismatch: in dir '{message_id}' but references '{internal_message_id}'"
                                )

                            # Check if internal messageID exists
                            if (
                                internal_message_id
                                and internal_message_id not in valid_messages
                            ):
                                issues.append(
                                    f"messageID '{internal_message_id}' doesn't exist"
                                )
            ...
            if "notification.v1" in cache_data:
                try:
                    ...
                    notif_data["list"] = [
                        n
                        for n in notif_data.get("list", [])
                        if n.get("session", "").startswith("ses_")
                        and n.get("session") in valid_sessions
...
def check_existing_instance(port: int) -> bool:
    ...
    try:
        req = urllib.request.Request(
            f"http://localhost:{port}/api/storage-path", method="GET"
        )
        with urllib.request.urlopen(req, timeout=2) as response:
            if response.status == 200:
                ...
                if choice == "2":
                    ...
                    print(
                        "Warning: Port may still be in use. Attempting to start anyway...\n"
                    )
                    return True
...
def main():
    """Main entry point for the OpenCode Session Manager application."""
    PORT = 8765
